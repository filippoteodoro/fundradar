...
├── 3. translate          (DeepL→Azure — MUST run before filter; filter uses English patterns)
...
└── 10. enrich_portfolio_final (optional — second Gemini pass for entries that step 9 added)
```

The step list is `STEPS` in `apps/worker/fundradar_worker/pipeline.py`.
...
LinkedIn       →  linkedin/*.py            →  linkedin/fund_people_stats.json
...
| `data/derived/linkedin/fund_people_stats.json` | `getTeamAnalyticsForFund()` | `/funds/[slug]` (team) |
...
| `data/derived/fund_aliases.json` | `slug_normalizer.py` (worker) | none — canonicalizes and blocks slugs during ingestion |
...
- RSS/news signals can involve more than one fund. Worker output stores this in
  `related_fund_slugs` and still emits one row per primary `fund_slug`.
...
All in `data/derived/`. Most are gitignored, so a fresh clone does not have them; the worker creates them on the first run.

- `snapshots.json` — content snapshots for change detection
- `circuit_state.json` — circuit breaker state
- `url_status.json` — URL health tracking (committed)
- `monitor_checkpoint.json` — last URL, saved on graceful shutdown
