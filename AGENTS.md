...
A free, public directory of Italian PE/VC funds with monitored "signals" (news, hires, deals). The value is clean, searchable data with source citations. All data is freely accessible — no account required. It is a free, open-source project with no payments.
...
20. **New sector tag in portfolio data breaks CI** → `sectorGroups.test.ts` scans all current `portfolio_items.json` entries and fails if any sector tag is unmapped. **Root cause is almost always a scraper passing through a business description as a sector.** Fix the data at the source: (a) find the bad entry in `portfolio_items.json` and set its sector to the correct canonical value, (b) if the extractor is the source, add a length/word-count guard (>50 chars or >5 words → `None`) so descriptions don't become sectors, (c) `normalize_sectors.py` also has this guard (`is_garbage_sector`) — make sure the threshold there is tight. **Do NOT paper over garbage sectors by adding them to `SECTOR_TAG_ALIASES` — that hides the bug.** Only add to `SECTOR_TAG_ALIASES` for real sector labels that happen to be spelled differently (e.g. "fintech" → "Financial Services").
...
26. **CI new-fund gate fires on routine pipeline runs** → `scripts/check_new_fund_gate.py` triggers whenever `db.json` or extractor files change. If all changed funds already existed in db.json (pipeline just updated fields), they are treated as **established** and skip strict data-completeness checks (portfolio enrichment 100%, signal parity, signal completeness, Italy asset verification). Only genuinely new db.json entries (no prior slug) face full checks. For established-only changesets: the gate passes without requiring any artifact files (`gemini_fund_asset_audit.json`, `new_fund_completion_report.json` are both gitignored and never present on CI for routine commits). For changesets with genuinely new funds: both artifact files must be in the commit diff — touch the audit file's `last_gate_verification` field to satisfy the freshness check.
...
29. **CSS-hide responsive pattern (AI slop)** → AI agents default to rendering both mobile and desktop variants simultaneously and hiding one via CSS classes (`.mobile-hide`/`.mobile-show`). `FundsTable.tsx` had this bug — it already had a correct `isMobile` state (via `useEffect` + `matchMedia('(max-width: 768px)')`) but was ALSO rendering both `<span>` elements. Fixed: use `{isMobile ? shortText : fullText}` conditional rendering. When adding any responsive text variant, always check whether `isMobile` state already exists — never reach for CSS-hiding of duplicate DOM nodes. Recharts (`TeamAnalyticsCharts.tsx`) and Leaflet (`LeafletMap.tsx`) are both already correctly code-split via `dynamic()` with `ssr: false`.
...
- **Signal importance is computed at runtime** — `SignalsFeed.tsx:getImportanceScore()` computes from quality_score + evidence_score + type bonus + fund priority + Italy bonus. NOT stored in JSON. Don't add an `importance` field to the pipeline output — it's by design.
...
GitHub holds every committed file. Gitignored files exist only in your local working copy, so back them up yourself.

**Committed files**: source code, `data/db.json`, `data/derived/portfolio_items.json`, `data/derived/company_profiles.json`, `data/derived/detected_signals_*.json`, `data/derived/pem_deals.json`, `data/derived/fund_people_stats.json`, `data/derived/gemini_fund_asset_zero_italy_verified.json`.

**Gitignored files** (local only, not on GitHub):

| File / Pattern | Why gitignored | Risk if lost |
|----------------|---------------|-------------|
| `data/derived/linkedin/raw/` | Raw scraped data, Apify cost to regenerate | High — no GitHub copy |
| `data/derived/gemini_fund_asset_audit*.json` | Large/noisy intermediate audit output | Medium — the derived output (`zero_italy_verified.json`) is committed |
| `data/models/` | ML classifier joblib files | Low — can be retrained |
| `data/pem/*.pdf` | Large source PDFs | Low — only needed to re-ingest PEM |
...
**IMPORTANT — "NEVER DELETE" warnings in these docs apply to gitignored files.** They are NOT recoverable from GitHub. A `git clean -fdx` wipes them permanently.
