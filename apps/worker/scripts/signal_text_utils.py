...
def fix_spacing(text: str) -> str:
    ...
    cleaned = re.sub(r"\bUp\s*Surge\s*on\b", "UpSurgeOn", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\bBe\s*Newtral\b", "BeNewtral", cleaned, flags=re.IGNORECASE)
    ...
    cleaned = re.sub(r"\bNan\s+o\s*prom\b", "Nanoprom", cleaned, flags=re.IGNORECASE)
    # Fix numeric magnitude merge (e.g. "140a fine 2026" -> "140 a fine 2026")
    # Italian "a" (to/at) or "e" (and) merged with preceding number
    cleaned = re.sub(r"\b(\d+)([ae])\b", r"\1 \2", cleaned)
