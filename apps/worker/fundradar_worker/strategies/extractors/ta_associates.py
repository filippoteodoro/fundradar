"""Site-specific extractors for www.ta.com (TA Associates).

TA Associates is a global growth equity firm (~$65B AUM, HQ Boston). Italian
portfolio companies include Nactarome (Milan) and Surfaces Group (Cenate Sotto).

Portfolio at /portfolio/investments/, news at /news/.
"""
...
def extract_portfolio(html: str, base_url: str) -> list[dict]:
    """Extract portfolio companies from TA Associates investments page."""
    soup = BeautifulSoup(html, "html.parser")
    companies = []
    seen = set()

    for item in soup.select(
        "article, .portfolio-item, .investment, .card, .company-card, "
        ...
        if link and "ta.com" not in link.get("href", ""):
        ...
        sector = None
        sector_el = item.select_one(".sector, .industry, .category")
        if sector_el:
...
def extract_news(html: str, base_url: str) -> list[dict]:
    """Extract news from TA Associates news page."""
    soup = BeautifulSoup(html, "html.parser")
    news = []
    seen = set()
